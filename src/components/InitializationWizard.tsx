...
export default function InitializationWizard({ open, onClose, onComplete, onFinalized }: Props) {
  ...
  const startReading = () => { if (!readingStartedAt) { setReadingStartedAt(Date.now()); setReadingRemaining(10); } };
  const onLogoFile = (file: File | undefined) => {
    if (!file) return;
    if (file.size > 2 * 1024 * 1024) { setLogoError('图片不能超过 2MB'); return; }
    setLogoError('');
  ...
  const finish = async () => {
    ...
    try {
      const result = await onComplete(buildInitializationData({ mode, province, schoolName, school, termStart, weekMode, excludeOfficialHolidays, scheduleMode, schoolLogo: logoDataUrl, subjectTrackModeEnabled: subjectTrackMode, seo: seoDraft }), { currentPassword: passwordDraft.current, newPassword: passwordDraft.next });
  ...
  return <div className="init-overlay" role="dialog" aria-modal="true" aria-labelledby="init-title"><div className="init-window">
    ...
    <main className="init-body">
      {step === 0 && <div className="init-form"><label><span>省份 / 地区</span><InlineSelect className="init-inline-select" value={province} onChange={setProvince} options={[{ value: '', label: '请选择省份或地区' }, ...CHINA_PROVINCES.map(item => ({ value: item, label: item }))]} /></label><label><span>学校名称</span><input value={schoolName} onChange={event => setSchoolName(event.target.value)} placeholder="如：第一中学" maxLength={80} /></label><div className="init-seo"><strong>学校图标（可选）</strong><div className="init-logo-row">{logoDataUrl ? <img className="init-logo-preview" src={logoDataUrl} alt="学校图标" /> : <span className="init-logo-empty">未上传</span>}<input type="file" accept="image/*" onChange={event => onLogoFile(event.target.files?.[0])} />{logoDataUrl && <button type="button" onClick={() => setLogoDataUrl("")}>移除</button>}</div>{logoError && <p className="init-note">{logoError}</p>}<strong>站点信息（SEO，可选）</strong><label><span>浏览器标题后缀</span><input value={seoDraft.titleSuffix} onChange={event => setSeoDraft(value => ({ ...value, titleSuffix: event.target.value }))} placeholder="如：考试看板" maxLength={60} /></label><label><span>SEO 描述</span><input value={seoDraft.description} onChange={event => setSeoDraft(value => ({ ...value, description: event.target.value }))} placeholder="一句话介绍本校考试看板，将用于搜索引擎描述" maxLength={200} /></label><label><span>关键词</span><input value={seoDraft.keywords} onChange={event => setSeoDraft(value => ({ ...value, keywords: event.target.value }))} placeholder="学校名称, 考试安排, 教室大屏" maxLength={120} /></label><label><span>站点公开地址</span><input value={seoDraft.siteUrl} onChange={event => setSeoDraft(value => ({ ...value, siteUrl: event.target.value }))} placeholder="https://exam.example.edu.cn（用于搜索引擎收录，可稍后修改）" maxLength={200} /></label></div><div className="init-school-fullname"><span>完整校名</span><strong>{schoolFullName(province, schoolName) || '选择省份并填写学校名称后自动生成'}</strong></div><p className="init-note">完整校名将显示在考试安排预览和 A4 PDF 中，并在你同意遥测后与省份一起上报作者端。</p></div>}
...
{step === 2 && <div className="init-form"><label><span>学期开始日期</span><DateTimeField className="init-date-time-field" value={termStart} onChange={setTermStart} mode="date" title="选择学期开始日期" showFieldPreview={false} /></label><label><span>周次模式</span><InlineSelect className="init-inline-select" value={weekMode} onChange={value => setWeekMode(value as WeeklyWeekMode)} options={[{ value: 'single', label: '统一周表' }, { value: 'ab', label: 'A/B 周交替' }]} /></label><label className="init-check"><input type="checkbox" checked={excludeOfficialHolidays} onChange={event => setExcludeOfficialHolidays(event.target.checked)} /><span className="init-check__text"><strong>自动排除法定节假日</strong><small>生成周测计划和日历预览时自动跳过系统内置的法定节假日。</small></span></label><label><span>默认运行模式</span><InlineSelect className="init-inline-select" value={scheduleMode} onChange={value => setScheduleMode(value as ScheduleMode)} options={[{ value: 'major-only', label: '仅大型考试' }, { value: 'weekly-only', label: '仅周测' }, { value: 'automatic', label: '自动调度' }]} /></label><label className="init-check"><input type="checkbox" checked={subjectTrackMode} onChange={event => setSubjectTrackMode(event.target.checked)} /><span className="init-check__text"><strong>分科模式</strong><small>开启后已分科班级按选科过滤，未分科班级读取全部 9 门；关闭后所有科目按考试范围直接下发。默认关闭。</small></span></label></div>}
      ...
      {step === 5 && <div className="init-documents"><p className="init-documents__lead">建议在继续前查看使用文档。此步骤不强制打开链接；若浏览器阻止弹窗，可稍后从“系统公告 → 文档”手动获取。</p>{documentsLoading ? <div className="init-documents__state">正在加载文档…</div> : validDocuments.length ? <>{embedUrl && <iframe className="init-documents__frame" src={embedUrl} title="使用文档" onLoad={startReading} />}<div className="init-documents__list">{validDocuments.map(document => <article key={document.id}><div><strong>{document.title || '使用文档'}</strong>{document.summary && <div className="md-body" dangerouslySetInnerHTML={{ __html: renderMarkdown(document.summary) }} />}</div><button type="button" onClick={() => openDocument(document)}>{document.buttonLabel?.trim() || '打开文档'} ↗</button></article>)}</div></> : <div className="init-documents__state">暂时没有可用文档，可继续初始化并稍后在系统公告中查看。</div>}{docHint && <p className="init-documents__error" role="alert">{docHint}</p>}<label className="init-check init-documents__confirm"><input type="checkbox" checked={docReadConfirmed} onChange={event => setDocReadConfirmed(event.target.checked)} /><span className="init-check__text"><strong>我已打开文档并停留至少 10 秒</strong><small>未勾选时下一步会提示一次，仍可跳过。</small></span></label>{documentsError && <p className="init-documents__error">{documentsError}</p>}{(documentRead || docReadConfirmed) && <div className="init-documents__done"><strong>已查看文档</strong><span>今后可从“首页 → 系统公告 → 文档”，或“管理后台 → 更多 → 查看公告 → 文档”再次查找使用文档。</span></div>}</div>}
  ...
  if (step === 5) { if (!documentRead && !docReadConfirmed) { setDocHint('请先打开并阅读使用文档（停留 10 秒），或勾选确认；仍可再次点击“继续”跳过。'); return; } setStep(6); return; }
