...
const TONES: Record<ThemeKey, Record<AlertState, { accent: string; soft?: string }>> = {
  ...
  clean: {
    '15min': { accent: '#d38b2f', soft: '#fbf1e2' },
    end15: { accent: '#d38b2f', soft: '#fbf1e2' },
    '5min': { accent: '#d0483a', soft: '#fbe7e4' },
    start: { accent: '#146eb4', soft: '#e6f4ff' },
    ended: { accent: '#2f7d59', soft: '#e5f1ea' },
    next: { accent: '#6b57b8', soft: '#efe9f8' },
  },
  chalk: {
    '15min': { accent: '#f7e59b' },
    end15: { accent: '#f7e59b' },
  ...
  sunrise: {
    '15min': { accent: '#d58b25', soft: '#fff1ca' },
    end15: { accent: '#d58b25', soft: '#fff1ca' },
    '5min': { accent: '#d95048', soft: '#fde8e5' },
    start: { accent: '#2b9bcb', soft: '#e2f5ff' },
    ended: { accent: '#66869d', soft: '#e7f0f5' },
    next: { accent: '#6670bd', soft: '#e9eaff' },
  },
  palette: {
    '15min': { accent: '#d7832d', soft: '#fff0d7' },
    end15: { accent: '#d7832d', soft: '#fff0d7' },
    '5min': { accent: '#d85148', soft: '#fbe7e4' },
    start: { accent: '#258f82', soft: '#dcf6ef' },
    ended: { accent: '#70849a', soft: '#eaf0f5' },
    next: { accent: '#6c64c0', soft: '#eeebff' },
  },
  orbit: {
    '15min': { accent: '#ce8b29', soft: '#fff2d4' },
    end15: { accent: '#ce8b29', soft: '#fff2d4' },
    '5min': { accent: '#cf5850', soft: '#fde9e6' },
    start: { accent: '#23806e', soft: '#dff4ec' },
    ended: { accent: '#587a80', soft: '#e7f2f0' },
    next: { accent: '#536fc5', soft: '#e8eeff' },
  },
  peach: {
    '15min': { accent: '#d77d42', soft: '#fff0e7' },
    end15: { accent: '#d77d42', soft: '#fff0e7' },
    '5min': { accent: '#d85650', soft: '#fde8e6' },
    start: { accent: '#478f82', soft: '#e0f3ed' },
    ended: { accent: '#717a8a', soft: '#eef0f4' },
    next: { accent: '#637cc9', soft: '#eceeff' },
  },
  poster: {
    '15min': { accent: '#d39a22', soft: '#fff3c8' },
    end15: { accent: '#d39a22', soft: '#fff3c8' },
    '5min': { accent: '#df5b46', soft: '#ffeae4' },
    start: { accent: '#278f85', soft: '#ddf3ef' },
    ended: { accent: '#5c7483', soft: '#eaf0f4' },
    next: { accent: '#5e67bd', soft: '#ececff' },
  },
  ice: {
    '15min': { accent: '#d48335', soft: '#fff0db' },
    end15: { accent: '#d48335', soft: '#fff0db' },
    '5min': { accent: '#db6254', soft: '#ffe9e5' },
    start: { accent: '#2585a8', soft: '#e1f5ff' },
    ended: { accent: '#668293', soft: '#eaf1f6' },
    next: { accent: '#5c71c2', soft: '#e9edff' },
  },
  quartz: {
    '15min': { accent: '#f0be5b' },
    end15: { accent: '#f0be5b' },
...
export default function ExamAlertOverlay({ item, now, designId, masterTitle, timeSynced }: Props) {
    ...
    return (
      <div className={rootCls} style={styleVars} role="alert" aria-live="assertive">
        ...
        <div className="eao-emg-bar eao-emg-bar--bottom">
          <FitText as="span" className="eao-emg-sub">
            {item.subtext}
          </FitText>
        </div>
      </div>
    ...
    return (
      <div className={rootCls} style={styleVars} role="alert" aria-live="assertive">
        ...
        <div className="eao-edit-main">
          <FitText className="eao-edit-kicker">
            {brand} / {year}
          </FitText>
          <FitText className="eao-edit-en">
            {EN_LABEL[item.tone]} / {item.label}
          </FitText>
          <FitText className="eao-title">{item.title}</FitText>
          <FitText className={`eao-hero ${heroSizeClass(hero)}`}>{hero}</FitText>
  ...
  return (
    <div className={rootCls} style={styleVars} role="alert" aria-live="assertive">
      {map.theme === 'cmd' && (
        <>
          <span className="eao-side eao-side--l" />
          <span className="eao-side eao-side--r" />
        </>
      )}
      <div className="eao-topbar">
        <span className="eao-master">{masterTitle || '考试提醒'}</span>
        <span className={`eao-sync${timeSynced ? ' is-ok' : ''}`}>
          <span className="eao-sync-dot" />
          {syncText}
        </span>
      </div>
