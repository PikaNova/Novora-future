...
const SCOPE_LABEL: Record<DesignRuleScope, string> = {
  school: '全校设计',
  grade: '年级',
  class: '班级',
  device: '设备实例',
...
export default function DesignPolicyManager({
  grades,
  classes,
  devices,
  canEdit,
  ...
  useEffect(() => {
    void fetchExamsFromServer().then((payload) => {
      if (payload?.designPolicy) setPolicy(normalizeDesignPolicy(payload.designPolicy));
    });
  }, []);
  useEffect(() => {
    ...
    () =>
      scope === 'grade'
        ...
        : scope === 'class'
          ? classes.map((item) => ({
              value: item.id,
              label: `${grades.find((grade) => grade.id === item.gradeId)?.name || '未分年级'} · ${item.name}`,
            }))
          : scope === 'device'
            ? devices.map((item) => ({
                value: item.instanceId,
                label: `${classes.find((entry) => entry.id === item.classId)?.name || '未绑定'} · ${item.instanceId.slice(0, 12)}`,
              }))
            : [{ value: '*', label: '全校考试端' }],
  ...
  const persist = async (rules: DesignAssignmentRule[]) => {
    ...
    try {
      ...
      notify('error', error instanceof Error ? error.message : '设计规则保存失败');
    } finally {
      setSaving(false);
    }
  };

  const targetLabel = (rule: DesignAssignmentRule) =>
    rule.scope === 'school'
      ? '全校考试端'
      : rule.scope === 'grade'
        ? grades.find((item) => item.id === rule.scopeId)?.name || rule.scopeId
        : rule.scope === 'class'
          ? (() => {
  ...
  const addRule = async () => {
    if (policy.rules.some((rule) => rule.scope === 'school')) {
      notify('warning', SCHOOL_LOCK_MESSAGE);
      return;
    }
    if (!scopeId) {
      notify('warning', '请先选择应用对象');
      return;
    }
    ...
    const nextRules =
      scope === 'school'
        ? [next]
        : [...policy.rules.filter((rule) => !(rule.scope === scope && rule.scopeId === scopeId)), next];
    const covering = coveringRuleFor(next);
    const coveredExisting =
      scope === 'school'
        ? []
        : policy.rules.filter((rule) => rule.id !== next.id && coveringRuleFor(rule, nextRules)?.id === next.id);
  ...
  return (
    <section className="design-policy-manager">
      <header>
        <span>
          <Palette aria-hidden="true" />
          <strong>考试端设计下发</strong>
        </span>
        <small>全校设计 &gt; 年级 &gt; 班级 &gt; 单独设备 &gt; 本地设置</small>
      </header>
      <div className="design-policy-manager__form">
        {schoolRule ? (
          <button
            type="button"
            className="design-policy-manager__lock"
            disabled={!canEdit || saving}
            onClick={() => notify('warning', SCHOOL_LOCK_MESSAGE)}
          >
            全校设计正在覆盖所有考试端。需要设置其他范围时，请先删除下方全校设计。
          </button>
        ) : (
          <>
            <InlineSelect
              value={scope}
              onChange={(value) => setScope(value as DesignRuleScope)}
              options={(Object.keys(SCOPE_LABEL) as DesignRuleScope[]).map((value) => ({
                value,
            ...
            <InlineSelect
              ...
              onChange={setScopeId}
              options={targets}
              disabled={!canEdit || saving || !targets.length}
              placeholder="选择应用对象"
            />
            <InlineSelect
              value={designId}
            ...
            <button
              ...
              disabled={!canEdit || saving || !scopeId}
              onClick={addRule}
            >
              {saving ? '保存中…' : '下发设计'}
            </button>
      ...
      {policy.rules.length > 0 && (
        <div className="design-policy-manager__rules">
          {policy.rules.map((rule) => {
            const covering = coveringRuleFor(rule);
            return (
              <div className={covering ? 'is-overridden' : ''} key={`${rule.scope}:${rule.scopeId}`}>
                <span>
                  <strong>
                    {SCOPE_LABEL[rule.scope]} · {targetLabel(rule)}
                  </strong>
                  <small>{DESIGNS.find((item) => item.id === rule.designId)?.name || rule.designId}</small>
                  {covering && (
                    <small className="design-policy-manager__status">
                      当前不生效 · 被 {SCOPE_LABEL[covering.scope]}“{targetLabel(covering)}”覆盖
                    </small>
                  )}
                </span>
                {canEdit && (
                  <button
                    ...
                    title="删除规则"
                    disabled={saving}
                    onClick={() => void persist(policy.rules.filter((item) => item !== rule))}
                  >
