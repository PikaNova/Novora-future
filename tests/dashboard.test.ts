...
import { getShanghaiDateKey } from '../src/utils/weeklySchedule.js';
...
const majors: DashboardMajor[] = [
  {
    id: 'm-all',
    name: '全校考试',
    items: [item('i-all', '语文', todayKey, '11:00', '13:00')],
  },
  {
    id: 'm-g1',
    name: '高一大考',
  ...
  },
  {
    ...
    name: '高二大考',
    targetGradeIds: ['g2'],
    items: [item('i-g2', '物理', todayKey, '16:00', '17:00')],
  },
  {
    id: 'm-ended',
    name: '已结束考试',
    items: [item('i-ended', '历史', todayKey, '07:00', '08:00')],
  },
];
...
test('grade scope includes class-targeted items in its grade and excludes other grades', () => {
  const items = collectScopedItems(majors, grades, classes, gradeAdminG1);
  assert.equal(
    items.some((item) => item.id === 'i-c1'),
    true,
  );
  assert.equal(
    items.some((item) => item.id === 'i-g2'),
    false,
  );
});
...
test('major endedAt moves items out of ongoing/upcoming and into ended', () => {
  const endedMajor: DashboardMajor = {
    ...majors[0],
    endedAt: now - 1_000,
    items: [{ ...majors[0].items[0], id: 'i-ended-now' }],
  };
  ...
  assert.equal(
    buildRecentEnded(items, now).some((entry) => entry.id === 'i-ended-now'),
    true,
  );
});
...
test('grade distribution counts school-wide exams for every grade and scopes grade admins', () => {
  ...
  const scoped = buildGradeDistribution(
    collectScopedItems(majors, grades, classes, gradeAdminG1),
    grades,
    actorVisibleGradeIds(gradeAdminG1, grades),
  );
...
test('scopedDevices filters by grade scope, class scope, revoked and management', () => {
  const gradeDevices = scopedDevices(devices, gradeAdminG1, classes)
